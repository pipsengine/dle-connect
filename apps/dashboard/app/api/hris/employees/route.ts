...
const ROLE_BY_CANONICAL: Record<string, Role> = {
  ...
  PAYROLLADMINISTRATOR: 'Payroll Officer',
  PAYROLLSUPERVISOR: 'Payroll Officer',
  HRADMINISTRATOR: 'HR Director',
...
const toProfileOverride = (employeeId: string, draft: EmployeeDraftPayload) => {
  ...
  const jobDetails: Record<string, string | null> = {
    ...
    location: draft.job?.officeLocation || draft.employment?.workLocation || null,
    officeSite: draft.job?.officeLocation || null,
    workCenter: draft.job?.workCenter || null,
  ...
  try {
    ...
    for (const [overrideId, override] of storeOverrides.entries()) {
      ...
      directoryRows.unshift({
        ...
        officeLocation: p.jobDetails?.officeLocation || p.jobDetails?.officeSite || '',
        workCenter: p.jobDetails?.workCenter || '',
