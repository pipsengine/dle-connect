...
export default function LeaveApprovalAuthorizeClient() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      setError('');
      try {
        const res = await fetch(`/api/hris/leave/leave-approval/email-action?token=${encodeURIComponent(token)}`, { cache: 'no-store' });
        const json = (await res.json()) as ApiResponse<PreviewPayload>;
        if (!res.ok || json.status !== 'success' || !json.data) throw new Error(json.error || 'Unable to load leave approval request.');
        setPreview(json.data);
        if (json.data.requiresLogin) {
          const next = `/workforce-portal/leave-approval/authorize?token=${encodeURIComponent(token)}`;
          router.replace(`/login?next=${encodeURIComponent(next)}`);
        }
      } catch (event) {
        setError(event instanceof Error ? event.message : 'Unable to load leave approval request.');
      } finally {
        setLoading(false);
      }
    })();
  ...
  const confirm = async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/hris/leave/leave-approval/email-action', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ token, note }),
      });
      const json = (await res.json()) as ApiResponse<{ status: string; leaveType: string; decision: string }>;
      if (!res.ok || json.status !== 'success' || !json.data) throw new Error(json.error || 'Unable to complete leave approval.');
      setDone(`${json.data.decision === 'approve' ? 'Approved' : 'Rejected'} — leave is now ${json.data.status}.`);
    } catch (event) {
      setError(event instanceof Error ? event.message : 'Unable to complete leave approval.');
    } finally {
      setPosting(false);
    }
  };
  ...
  if (loading) {
    return <div className="mx-auto max-w-2xl px-6 py-16 text-sm font-semibold text-slate-600">Verifying authenticated leave approval link…</div>;
  }
    ...
    <div className="min-h-screen bg-slate-50 px-6 py-10">
      <div className="mx-auto max-w-2xl rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-950">Secure Leave Approval</h1>
            <p className="mt-1 text-sm font-semibold text-slate-600">Authenticated approval is required before this leave action can be completed.</p>
          </div>
        </div>
        ...
        {preview && !done ? (
          <div className="mt-6 space-y-4">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <p className="mt-2 text-lg font-black text-slate-950">{preview.employeeName}</p>
              <p className="text-sm font-semibold text-slate-600">{preview.leaveType} · {preview.stageLabel}</p>
              <p className="text-sm font-semibold text-slate-600">Current status: {preview.status}</p>
              <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                <div className="rounded-xl bg-white p-3"><p className="text-xs font-bold uppercase text-slate-500">Period</p><p className="font-black">{preview.startDate} to {preview.endDate}</p></div>
                <div className="rounded-xl bg-white p-3"><p className="text-xs font-bold uppercase text-slate-500">Days</p><p className="font-black">{preview.days}</p></div>
                <div className="rounded-xl bg-white p-3"><p className="text-xs font-bold uppercase text-slate-500">Reliever</p><p className="font-black">{preview.relieverName || 'Not configured'}</p></div>
            ...
            {!preview.statusOk ? (
              <p className="text-sm font-bold text-amber-700">This leave request has already moved to another approval stage.</p>
            ) : null}
          </div>
        ) : null}
      </div>
