...
export default function FinancePostingClient({ initialWorkspace }: Props) {
  ...
  const kpis = [
    { id: 'ready' as TabId, label: 'Ready to mark posted', value: String(workspace.summary.readyToPost), detail: money(workspace.summary.readyValue), icon: Wallet, wrap: 'bg-sky-50', color: 'text-sky-600' },
    { id: 'all' as TabId, label: 'Open on desk', value: String(workspace.rows.length), detail: 'Not yet marked posted', icon: Clock3, wrap: 'bg-amber-50', color: 'text-amber-600' },
    { id: 'notReady' as TabId, label: 'Needs review', value: String(workspace.summary.notReady), detail: 'Not ready / incomplete', icon: CheckCircle2, wrap: 'bg-slate-100', color: 'text-slate-600' },
    { id: 'all' as TabId, label: 'With documents', value: String(workspace.summary.withDocuments), detail: 'Supporting files present', icon: FileText, wrap: 'bg-violet-50', color: 'text-violet-600' },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {kpis.map((card) => (
          <button
            ...
            onClick={() => {
              setTab(card.id);
              document.getElementById('finance-posting-list')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
            }}
      ...
      <section id="finance-posting-list" className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
