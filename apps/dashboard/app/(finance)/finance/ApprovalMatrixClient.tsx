    ...
    <div className="space-y-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm text-slate-500">
              Configure approval strategies and workflows based on payment type, amount thresholds and organisational structure.
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <section className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 rounded-xl border border-emerald-100 bg-emerald-50 px-3 py-2 text-sm text-emerald-800">
            {workspace.rules.length
              ...
              : 'No approval rules configured yet. Create rules to drive Cash Advance and Supplier Invoice workflows.'}
          </div>
          <ul className="mt-3 space-y-2 text-sm text-slate-600">
            {[
              ['Sage X3', workspace.source.includes('Enterprise') ? 'Available' : 'Offline'],
              ['Approval Rules Engine', 'Active'],
        ...
        <section className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
          ...
          <ul className="mt-3 space-y-2 text-sm text-slate-600">
            <li className="rounded-lg bg-slate-50 px-3 py-2">Company / entity filters</li>
            <li className="rounded-lg bg-slate-50 px-3 py-2">Reporting period</li>
            <li className="rounded-lg bg-slate-50 px-3 py-2">Actual / Budget / Forecast</li>
            <li className="rounded-lg bg-slate-50 px-3 py-2">Export and distribution</li>
          </ul>
        </section>
        ...
        <section className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
          ...
          <ul className="mt-3 space-y-2 text-sm text-slate-600">
            ...
            <li className="rounded-lg bg-slate-50 px-3 py-2">Drill-down audited</li>
      ...
      <section className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
        ...
        {workspace.rules.length ? (
                ...
                {workspace.rules.map((rule) => (
                  <tr key={rule.matrixId} className="border-t border-slate-100">
                    ...
                    <td className="px-3 py-2.5 tabular-nums">
                      ...
                      {' – '}
                      {rule.maxAmount == null ? 'Open' : new Intl.NumberFormat('en-NG').format(rule.maxAmount)}
                    </td>
          ...
          <p className="rounded-xl bg-slate-50 px-4 py-8 text-center text-sm text-slate-500">
            No rules in the database yet. Use Create approval rule to write the first band.
          </p>
        )}
      </section>
