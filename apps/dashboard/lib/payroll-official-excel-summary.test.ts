...
assert(usdReport?.columns.includes('USD Gross'), 'USD REPORT is a salary register');
...
assert(!usdReport?.columns.includes('Date of Birth'), 'USD REPORT omits date of birth');
assert(!usdReport?.columns.includes('Gender'), 'USD REPORT omits gender');
assert(!usdReport?.columns.includes('Age'), 'USD REPORT omits age');
assert(!usdExport.some((sheet) => /BANK/i.test(sheet.sheetName || '')), 'DLE USD salary export omits bank details');
...
const codeIdx = usdReport?.columns.indexOf('Employee Code') ?? -1;
const mdRow = (usdReport?.rows || []).find((row) => String(row[codeIdx] ?? '').includes('P0413') || String(row[0] ?? '').includes('P0413'));
const nayakRow = (usdReport?.rows || []).find((row) => String(row[codeIdx] ?? '').includes('PEX001') || String(row[0] ?? '').includes('PEX001'));
