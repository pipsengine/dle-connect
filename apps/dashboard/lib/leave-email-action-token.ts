...
export const createLeaveEmailActionToken = (input: {
  ...
  if (!payload.requestId || !payload.recipientEmail) throw new Error('Leave email action token requires requestId and recipientEmail.');
...
export const verifyLeaveEmailActionToken = (token: string) => {
  ...
  if (!encoded || !signature || sign(encoded) !== signature) throw new Error('Approval link is invalid.');
  const payload = JSON.parse(base64UrlDecode(encoded)) as LeaveEmailActionPayload;
  if (!payload?.requestId || !payload.recipientEmail || !payload.decision || !payload.approverKind) {
    throw new Error('Approval link is invalid.');
  }
  if (payload.exp < Date.now()) throw new Error('Approval link has expired.');
