/*
  DLE_Enterprise Performance Management foundation.
  Safe to re-run. Application ensurePerformanceSqlSchema() also bootstraps on first use.
*/
USE [DLE_Enterprise];
GO
...
BEGIN
  CREATE TABLE [hris].[PerformanceMigrations] (
    ...
    [RowCountsJson] NVARCHAR(MAX) NULL CONSTRAINT [CK_PerformanceMigrations_RowCountsJson] CHECK ([RowCountsJson] IS NULL OR ISJSON([RowCountsJson]) = 1),
...
BEGIN
  CREATE TABLE [hris].[PerformanceDomainDocuments] (
    ...
    [DomainJson] NVARCHAR(MAX) NOT NULL CONSTRAINT [CK_PerformanceDomainDocuments_DomainJson] CHECK (ISJSON([DomainJson]) = 1),
    [SourceHash] NVARCHAR(128) NULL,
...
BEGIN
  CREATE TABLE [hris].[PerformanceCycles] (
    ...
    [CycleJson] NVARCHAR(MAX) NOT NULL CONSTRAINT [CK_PerformanceCycles_CycleJson] CHECK (ISJSON([CycleJson]) = 1)
  );
  CREATE INDEX [IX_PerformanceCycles_Status] ON [hris].[PerformanceCycles] ([Status]);
  CREATE INDEX [IX_PerformanceCycles_Year] ON [hris].[PerformanceCycles] ([Year]);
END
GO
...
BEGIN
  CREATE TABLE [hris].[PerformanceCycleEligibility] (
    ...
    CONSTRAINT [UQ_PerformanceCycleEligibility_CycleEmployee] UNIQUE ([CycleId], [EmployeeId])
  );
  CREATE INDEX [IX_PerformanceCycleEligibility_CycleIncluded] ON [hris].[PerformanceCycleEligibility] ([CycleId], [Included], [Department]);
END
GO
...
BEGIN
  CREATE TABLE [hris].[PerformanceGoals] (
    ...
    [GoalJson] NVARCHAR(MAX) NOT NULL CONSTRAINT [CK_PerformanceGoals_GoalJson] CHECK (ISJSON([GoalJson]) = 1)
  );
  CREATE INDEX [IX_PerformanceGoals_CycleEmployeeStatus] ON [hris].[PerformanceGoals] ([CycleId], [EmployeeId], [Status]);
  CREATE INDEX [IX_PerformanceGoals_ManagerStatus] ON [hris].[PerformanceGoals] ([ManagerId], [Status]);
END
GO
...
BEGIN
  CREATE TABLE [hris].[PerformanceResults] (
    ...
    [ResultJson] NVARCHAR(MAX) NOT NULL CONSTRAINT [CK_PerformanceResults_ResultJson] CHECK (ISJSON([ResultJson]) = 1),
    CONSTRAINT [UQ_PerformanceResults_CycleEmployee] UNIQUE ([CycleId], [EmployeeId])
  );
  CREATE INDEX [IX_PerformanceResults_CycleStatusBand] ON [hris].[PerformanceResults] ([CycleId], [Status], [RatingBand]);
END
GO
...
BEGIN
  CREATE TABLE [hris].[PerformanceAssessments] (
    ...
    [AssessmentJson] NVARCHAR(MAX) NOT NULL CONSTRAINT [CK_PerformanceAssessments_AssessmentJson] CHECK (ISJSON([AssessmentJson]) = 1)
  );
  CREATE INDEX [IX_PerformanceAssessments_CycleTypeStatus] ON [hris].[PerformanceAssessments] ([CycleId], [AssessmentType], [Status]);
  CREATE INDEX [IX_PerformanceAssessments_EmployeeStatus] ON [hris].[PerformanceAssessments] ([EmployeeId], [Status]);
END
GO
...
BEGIN
  CREATE TABLE [hris].[PerformanceTasks] (
    ...
    [TaskJson] NVARCHAR(MAX) NOT NULL CONSTRAINT [CK_PerformanceTasks_TaskJson] CHECK (ISJSON([TaskJson]) = 1)
  );
  CREATE INDEX [IX_PerformanceTasks_AssigneeStatusDue] ON [hris].[PerformanceTasks] ([AssigneeId], [Status], [DueDate]);
END
GO
...
BEGIN
  ...
  CREATE INDEX [IX_PerformanceAuditTrail_Entity] ON [hris].[PerformanceAuditTrail] ([EntityType], [EntityId], [EventAt] DESC);
  CREATE INDEX [IX_PerformanceAuditTrail_EventAt] ON [hris].[PerformanceAuditTrail] ([EventAt] DESC);
END
GO
...
BEGIN
  CREATE TABLE [hris].[PerformanceAnalyticsSnapshots] (
    ...
    [SnapshotJson] NVARCHAR(MAX) NULL CONSTRAINT [CK_PerformanceAnalyticsSnapshots_SnapshotJson] CHECK ([SnapshotJson] IS NULL OR ISJSON([SnapshotJson]) = 1)
  );
  CREATE INDEX [IX_PerformanceAnalyticsSnapshots_CycleGenerated] ON [hris].[PerformanceAnalyticsSnapshots] ([CycleId], [GeneratedAt] DESC);
END
GO
...
BEGIN
  CREATE TABLE [hris].[PerformanceNavPreferences] (
    ...
    [PreferencesJson] NVARCHAR(MAX) NOT NULL CONSTRAINT [CK_PerformanceNavPreferences_Json] CHECK (ISJSON([PreferencesJson]) = 1),
...
BEGIN
  CREATE TABLE [hris].[PerformanceOutbox] (
    ...
    [PayloadJson] NVARCHAR(MAX) NOT NULL CONSTRAINT [CK_PerformanceOutbox_PayloadJson] CHECK (ISJSON([PayloadJson]) = 1),
  ...
  CREATE INDEX [IX_PerformanceOutbox_StatusCreated] ON [hris].[PerformanceOutbox] ([Status], [CreatedAt]);
END
GO
...
PRINT 'Performance Management schema present.';
GO
