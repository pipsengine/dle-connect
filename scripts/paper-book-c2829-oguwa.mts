/**
 * Paper-book C2829 Christain Oguwa on Akande Idi-Oro day sheets.
 *   2026-08-17 → 2026-09-22
 * Skip Eid Maulud 2026-08-25 and Sundays. Do not invent clocks.
 *
 *   npx tsx --tsconfig apps/dashboard/tsconfig.json scripts/paper-book-c2829-oguwa.mts
 *   npx tsx --tsconfig apps/dashboard/tsconfig.json scripts/paper-book-c2829-oguwa.mts --apply
 */
...
const FALLBACK_PROJECT = { code: 'DL2424', name: 'DL2424' };
const CODE = 'C2829';
const NAME = 'Christian Oguwa';
...
WHERE CONVERT(varchar(10), h.TimesheetDate, 23) = @date
  ...
  AND (
    ISNULL(h.LocationName, N'') LIKE N'%IDI%'
    OR ISNULL(h.LocationName, N'') LIKE N'%ORO%'
  )
ORDER BY
...
const employeeNameOnFile = async (pool: sql.ConnectionPool) => {
  const result = await pool.request().query(`
SELECT TOP 1 EmployeeName FROM [hris].[TimesheetLines]
WHERE EmployeeNo = N'C2829' AND ISNULL(EmployeeName, N'') <> N''
ORDER BY Id DESC
`);
  return compact(result.recordset[0]?.EmployeeName) || NAME;
};

const insertLine = async (pool: sql.ConnectionPool, headerId: string, name: string) => {
  const found = await pool.request()
    .input('headerId', sql.NVarChar(160), headerId)
    .query(`
SELECT TOP 1 Id FROM [hris].[TimesheetLines]
WHERE CONVERT(NVARCHAR(4000), HeaderId) = CONVERT(NVARCHAR(4000), @headerId)
  AND (EmployeeNo = N'C2829' OR EmployeeId = N'C2829')
ORDER BY CASE WHEN Id LIKE N'line-%' THEN 0 ELSE 1 END, Id
`);
  if (found.recordset[0]?.Id) return String(found.recordset[0].Id);

  const lineId = `line-${headerId}-C2829`;
  await pool.request()
...
VALUES
  (@id, @headerId, N'C2829', N'C2829', @employeeName, NULL, NULL, 0,
...
const main = async () => {
  ...
  const displayName = await employeeNameOnFile(pool);
...
`);
  ...
  for (let date = FROM; date <= TO; date = addDays(date, 1)) {
    if (weekday(date) === 0) {
      skipped += 1;
      report.push({ code: CODE, date, action: 'skipped', reason: 'Sunday' });
      continue;
    }
    if (HOLIDAYS.has(date)) {
      skipped += 1;
      report.push({ code: CODE, date, action: 'skipped', reason: 'public holiday (Eid Maulud)' });
      continue;
    }
    ...
    if (row) {
      ...
      if (locked) {
        skipped += 1;
        report.push({ code: CODE, date, action: 'skipped', reason: `timesheet ${row.Status} is locked` });
        continue;
      }
      if (Number(row.UsedHours || 0) > 0 || compact(row.ClockIn)) {
        skipped += 1;
        report.push({ code: CODE, date, action: 'skipped', reason: 'already present' });
        continue;
      }
    ...
    if (!header) {
      skipped += 1;
      report.push({ code: CODE, date, action: 'skipped', reason: 'No Akande Idi-Oro day sheet' });
      continue;
    }
    const locked = /hr_acknowledged|locked|approved/i.test(compact(header.Status).replace(/[\s-]+/g, '_'));
    if (locked) {
      skipped += 1;
      report.push({ code: CODE, date, action: 'skipped', reason: `timesheet ${header.Status} is locked` });
      continue;
    }
    const project = await majorityProject(pool, header.HeaderId);
    let lineId = `pending-${date}-${CODE}`;
    if (APPLY) {
      lineId = await insertLine(pool, header.HeaderId, displayName);
...
SELECT CONVERT(varchar(10), h.TimesheetDate, 23) AS d, h.Status,
...
WHERE l.EmployeeNo = N'C2829'
  AND CONVERT(varchar(10), h.TimesheetDate, 23) BETWEEN N'2026-08-17' AND N'2026-09-22'
  AND ISNULL(h.ShiftLabel, N'Day') NOT LIKE N'%Night%'
